    ...
    <main className="max-w-4xl mx-auto px-4 py-8 space-y-6">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 border-b border-slate-200 pb-4">
        <div>
          ...
          <p className="text-xs text-slate-500 mt-1">Select printable poster template for shop counter standee, window poster, or table sticker.</p>
        </div>

        <div className="flex flex-wrap items-center gap-2 text-xs font-bold">
          <button
            onClick={() => setTemplate('COUNTER')}
            className={`px-3 py-2 rounded-xl transition ${template === 'COUNTER' ? 'bg-emerald-600 text-white shadow-xs' : 'bg-slate-100 text-slate-700 hover:bg-slate-200'}`}
          >
            Counter Standee
          </button>
          <button
            onClick={() => setTemplate('WINDOW')}
            className={`px-3 py-2 rounded-xl transition ${template === 'WINDOW' ? 'bg-emerald-600 text-white shadow-xs' : 'bg-slate-100 text-slate-700 hover:bg-slate-200'}`}
          >
            Window Banner
          </button>
          <button
            onClick={() => setTemplate('TABLE')}
            className={`px-3 py-2 rounded-xl transition ${template === 'TABLE' ? 'bg-emerald-600 text-white shadow-xs' : 'bg-slate-100 text-slate-700 hover:bg-slate-200'}`}
          >
            Table Sticker
          </button>
          ...
          <button
            ...
            className="px-3.5 py-2.5 rounded-xl bg-teal-600 hover:bg-teal-700 text-white font-bold flex items-center gap-1.5 transition ml-2 shadow-xs"
          >
          ...
          <button
            ...
            className="px-3.5 py-2.5 rounded-xl bg-slate-100 hover:bg-slate-200 text-slate-700 font-bold flex items-center gap-1.5 transition border border-slate-200"
          >
      ...
      <div className="flex justify-center py-4">
        {template === 'COUNTER' && (
          <div
            ref={posterRef}
            className="w-full max-w-sm bg-gradient-to-b from-slate-900 via-slate-900 to-slate-950 border-2 border-emerald-500/40 rounded-3xl p-8 text-center space-y-6 shadow-2xl text-white relative overflow-hidden"
          >
        ...
        {template === 'TABLE' && (
          <div
            ref={posterRef}
            className="w-full max-w-xs bg-slate-900 border-4 border-amber-500 rounded-3xl p-6 text-center space-y-4 shadow-2xl text-white"
          >
